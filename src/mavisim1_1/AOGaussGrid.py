...
#	ADDED
#	gauss_norm = gauss/np.amax(gauss)
#	ao_conv_core = signal.fftconvolve(psf_ramped, gauss_norm, mode="same")
#	ao_conv_core_norm = (ao_conv_core - np.amin(ao_conv_core))/(np.amax(ao_conv_core) - np.amin(ao_conv_core))
#	ao_conv_core_scale = ao_conv_core_norm * row["Flux"]
#
#   1. This class returns the field of source objects convolved with AO cores as well as a field of the raw Gaussians
#
...
from mavissimim.BilinearInterpolation import BilinearInterpolation
...
class AOGaussGrid:
	...
	def __init__(self, source_table, fv_psf):
	...
	def make_fields(self):
		...
		if self.fv_psf == False:
			...
			static_norm = static_psf/np.sum(static_psf)#(static_psf - np.amin(static_psf))/(np.amax(static_psf) - np.amin(static_psf))
