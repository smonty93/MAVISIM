...
def add_constant_sky(image, exp_time, filt_width, surf_bright=0):
	"""
	...
	if surf_bright != 0:
		# Assuming the surface bright is in 
		flux_jy = (3631 * 10**(-1*surf_bright/2.5))*u.Jy*u.nm**(-1)

		flux_ph_s_nm_cm2 = flux_jy * (1.51e3/mean_wavelength)

		flux_ph_s_nm_m2 = flux_ph_s_nm_cm2 * 10**4

		flux_ph_s_m2 = flux_ph_s_nm_m2 * filt_width #in nm
	...
	image_addsky = image + sky_value.value 
	
	return (image_addsky, sky_value.value)
